...
function parseSourceFromFile(filename) {
    return parseSourceFromString(fs.readFileSync(filename, 'utf-8'));
}

function parseSourceFromString(source) {
    var ast = recast.parse(source);
    return parseSourceFromAST(ast);
}

function parseSourceFromAST(ast) {
    var functions = [];
    recast.visit(ast, {
    ...
    function parseTestsForFunction(decl, stmt) {
        ...
        if (typeof decl.comments && decl.comments.length)
            tests = parseJsDoc(decl.comments[0].value);
        else if (typeof stmt.comments && stmt.comments.length)
            tests = parseJsDoc(stmt.comments[0].value);
        if (tests)
            functions.push({
                name: decl.id && decl.id.name,
                tests: tests
            });
    }
}
...
function parseExpression(expression) {
    ...
    do {
        if (match = NVP_ANNOTATION_REGEX.exec(expression)) {
            ...
            expression = expression.replace(NVP_ANNOTATION_REGEX, '');
        } else if (match = SIMPLE_ANNOTATION_REGEX.exec(expression)) {
            annotations[match[1].trim()] = true;
            expression = expression.replace(SIMPLE_ANNOTATION_REGEX, '');
        }
    } while (match);
